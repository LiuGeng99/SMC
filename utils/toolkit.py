...
def accuracy(y_pred, y_true, class_wise=True): # 做一下修改，不用新的和old  这个就默认给出total和对应的class wise的acc  
    ...
    if class_wise:
        ...
        for class_id in range(0, np.max(y_true)+1, increment):
            ...
            class_wise_acc[class_id] = np.around((y_pred[idxes] == y_true[idxes]).sum()*100/len(idxes), decimals=2)
    else:
        class_wise_acc = None
