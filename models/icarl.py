...
class iCaRL(BaseLearner):
    ...
    def after_task(self):
        self._old_network = self._network.copy().freeze() # 多一个network  所以占用空间会更大一些
        self._known_classes = self._total_classes
        logging.info("Exemplar size: {}".format(self.exemplar_size))
    
    def eval_task(self, test_loader, nme=False):
        if nme:
            y_pred, y_true = self._eval_nme(test_loader, self._class_means)
        else:
    ...
    def eval_task_seperate(self, nme=False): # 分别创建loader，然后分别测试
        cls_begin = 0
        cls_end = 0
        task_acc_dict = {}
        for task in range(self._cur_task + 1): # 之前的每一个task在当前epoch的性能
            ...
            if nme:
                y_pred, y_true = self._eval_nme(test_loader, self._class_means, class_index=np.arange(cls_begin, cls_end))
            else:
    ...
    def incremental_train(self):
        ...
        self.build_rehearsal_memory(self.data_manager, self.samples_per_class)
        
        # iCarl专用的NME评测
        nme_total_acc, nme_class_wise_acc = self.eval_task(self.test_loader, nme=True)
        self.nme_accs.append(nme_total_acc)
        logging.info(f"NME Curve: {self.nme_accs}")
